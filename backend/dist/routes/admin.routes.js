...
router.post('/showtimes/auto-generate', async (req, res) => {
    try {
        const { movieIds, movieId, theaterId, roomId, startDate, endDate, priceStandard = 85000, priceVip = 130000, priceDouble = 200000, priceRecliner = 150000, timeSlots, weekdaySlots, weekendSlots, blockbusterSlots, } = req.body;
        ...
        function parseSlot(s) {
            if (typeof s === 'number')
                return { hours: Math.floor(s), minutes: Math.round((s % 1) * 60) };
        ...
        function getSlotsForDay(dayOfWeek) {
            ...
            if (blockbusterSlots?.length)
                return blockbusterSlots;
            if (isWeekend)
                return weekendSlots?.length ? weekendSlots : DEFAULT_WEEKEND;
