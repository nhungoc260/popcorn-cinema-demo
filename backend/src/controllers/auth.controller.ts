...
const signAccess = (id: string, role: string, email: string) =>
  jwt.sign({ id, role, email }, process.env.JWT_ACCESS_SECRET!, { expiresIn: (process.env.JWT_ACCESS_EXPIRES || '8h') as any });

const signRefresh = (id: string) =>
  jwt.sign({ id }, process.env.JWT_REFRESH_SECRET!, { expiresIn: (process.env.JWT_REFRESH_EXPIRES || '30d') as any });
...
export async function verifyOtp(req: Request, res: Response) {
  try {
    ...
    const record = await OTP.findOne({ email, used: false, expiresAt: { $gt: new Date() } });
    if (!record) return res.status(400).json({ success: false, message: 'OTP hết hạn' });
    const bcryptV = require('bcryptjs');
    const isValidV = await bcryptV.compare(otp, record.otp);
    if (!isValidV) return res.status(400).json({ success: false, message: 'OTP không đúng' });
    record.used = true;
    await record.save();
...
export async function forgotPassword(req: Request, res: Response) {
  try {
    ...
    if (!user) return res.json({ success: true, message: 'Nếu email tồn tại, OTP đã được gửi' });
    const code = Math.floor(100000 + Math.random() * 900000).toString();
    const expiresAt = new Date(Date.now() + 10 * 60 * 1000);
