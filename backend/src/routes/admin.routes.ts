...
router.get('/dashboard', async (_, res) => {
  try {
    ...
    const [totalUsers, totalMovies, totalBookings, payments, todayBookings] = await Promise.all([
      ...
      Booking.countDocuments({ status: { $in: ['confirmed', 'checked_in'] }, createdAt: { $gte: today } }),
    ]);
    const revenue = payments.reduce((s: number, p: any) => s + p.amount, 0);

    const activeSTs = await Showtime.find({ isActive: true })
      .populate('room', 'totalSeats seats')
      .lean() as any[];
    const totalCapacity = activeSTs.reduce((s: number, st: any) => {
      // Ưu tiên seats.length (không tính aisle), rồi totalSeats, rồi fallback 0
      const seatsCount = st.room?.seats?.filter((s: any) => !s.isAisle)?.length
        || st.room?.totalSeats
        || 0;
      return s + seatsCount;
    }, 0);
    const totalBooked = activeSTs.reduce((s: number, st: any) => s + (st.bookedSeats?.length || 0), 0);
...
router.get('/users', async (req, res) => {
  ...
  const q: any = {};
  if (role) q.role = role;
  const users = await User.find(q).select('-password -refreshTokens')
...
router.get('/users/:id/detail', async (req, res) => {
  try {
    ...
    const loyalty = await Loyalty.findOne({ user: req.params.id }).lean();
    ...
    const totalSpent = payments.reduce((s: number, p: any) => s + p.amount, 0);
...
router.patch('/users/:id/status', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: updated,
      message: (updated as any).isActive ? 'Đã mở tài khoản' : 'Đã khoá tài khoản',
    });
  } catch (e: any) {
    res.status(500).json({ success: false, message: e.message });
  }
...
router.get('/theaters', async (_req, res) => {
  try {
    const theaters = await Theater.find().sort({ city: 1, name: 1 }).lean();
...
router.get('/rooms', async (req, res) => {
  try {
    ...
    const q: any = {};
    if (theaterId) q.theater = theaterId;
    const rooms = await Room.find(q).populate('theater', 'name city').lean();
    res.json({ success: true, data: rooms });
  } catch (e: any) { res.status(500).json({ success: false, message: e.message }); }
});
router.get('/rooms/:id', async (req, res) => {
  try {
    const room = await Room.findById(req.params.id).populate('theater', 'name city').lean();
...
router.post('/rooms', async (req, res) => {
  try {
    ...
    const totalSeats = (seats || []).filter((s: any) => !s.isAisle).length;
    const rows = [...new Set((seats || []).map((s: any) => s.row))].length;
    const cols = Math.max(...(seats || []).map((s: any) => s.number || 0), 0);
    const room = await Room.create({ theater, name, type: type || 'standard', seats, totalSeats, rows, cols, prices });
...
router.put('/rooms/:id', async (req, res) => {
  try {
    ...
    const update: any = { name, type };
    if (prices) update.prices = prices;
    if (seats) {
      update.seats = seats;
      update.totalSeats = seats.filter((s: any) => !s.isAisle).length;
      update.rows = [...new Set(seats.map((s: any) => s.row))].length;
      update.cols = Math.max(...seats.map((s: any) => s.number || 0), 0);
    }
...
router.post('/showtimes/auto-generate', async (req, res) => {
  try {
    const {
      movieIds, movieId, theaterId, roomId,
      startDate, endDate,
    ...
    const roomQuery: any = { theater: theaterId, isActive: true };
    if (roomId) roomQuery._id = roomId;
    const rooms = await Room.find(roomQuery).lean() as any[];
    if (!rooms.length) return res.status(404).json({ success: false, message: 'No rooms found for this theater' });

    const GOLDEN_HOURS = [14, 17, 19, 21];
    const ACTION_GENRES = ['action', 'thriller', 'sci-fi', 'horror'];

    function scoreSlot(hour: number, weekday: number, movie: any): number {
      const rating = movie.rating || 7;
      const genre  = movie.genre  || [];
      const isBlockbuster = rating >= 8 || genre.some((g: string) => ACTION_GENRES.includes(g.toLowerCase()));
      let score = 0;
      if (weekday === 5 || weekday === 6 || weekday === 0) score += 30;
      else if (weekday === 4) score += 15;
      else score += 5;
      if (hour >= 19 && hour <= 21) score += 40;
      else if (hour >= 14 && hour <= 17) score += 25;
      else if (hour >= 10 && hour <= 13) score += 10;
      else score += 5;
      if (GOLDEN_HOURS.includes(hour)) score += 20;
      if (rating >= 8.5) score += 20;
      else if (rating >= 7.5) score += 10;
      if (isBlockbuster && (weekday === 5 || weekday === 6)) score += 15;
      return score;
    }

    function hasOverlap(s1: Date, e1: Date, s2: Date, e2: Date): boolean {
      return s1 < e2 && e1 > s2;
    }
...
router.patch('/invoices/:id/status', async (req: any, res) => {
  try {
    ...
    const payment = await Payment.findByIdAndUpdate(
      req.params.id,
      { status, ...(status === 'success' ? { paidAt: new Date() } : {}) },
      { new: true }
    );
    if (!payment) return res.status(404).json({ success: false, message: 'Không tìm thấy hóa đơn' });

    // Đồng bộ trạng thái booking
    if (status === 'success') {
      await Booking.findByIdAndUpdate(payment.booking, { status: 'confirmed', paymentId: payment._id });
    } else if (status === 'failed' || status === 'refunded') {
      await Booking.findByIdAndUpdate(payment.booking, { status: 'pending' });
    }
