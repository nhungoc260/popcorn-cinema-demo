...
function buildHtml(otp: string, purpose: 'forgot' | 'verify'): string {
  ...
  const boxes = otp.split('').map(d =>
    `<td style="width:48px;height:60px;background:#1C1929;border:2px solid #A855F7;border-radius:12px;font-size:30px;font-weight:900;color:#A855F7;text-align:center;vertical-align:middle;font-family:monospace;">${d}</td>`
...
async function sendViaResend(to: string, subject: string, html: string, text: string): Promise<any> {
...
export async function sendOtpEmail(
  to: string,
  otp: string,
  purpose: 'forgot' | 'verify' = 'forgot'
): Promise<{ success: boolean; provider: string; previewUrl?: string }> {
