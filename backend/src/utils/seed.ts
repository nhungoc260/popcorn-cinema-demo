...
import { User, Movie, Theater, Room, Seat, Showtime, Coupon } from '../models';
...
async function seed() {
  ...
  // Clear
  await Promise.all([User.deleteMany({}), Movie.deleteMany({}), Theater.deleteMany({}), Room.deleteMany({}), Seat.deleteMany({}), Showtime.deleteMany({})]);
  console.log('🗑  Cleared existing data');

  // Users
  const admin = await User.create({ name: 'Ngọc Admin', email: 'ngocadmin@gmail.com', password: 'admin123', role: 'admin', isVerified: true });
  const staff = await User.create({ name: 'Ngọc Nhân Viên', email: 'ngocnhanvien@gmail.com', password: 'staff123', role: 'staff', isVerified: true });
  const customer = await User.create({ name: 'Ngọc User', email: 'ngocuser@gmail.com', password: 'user123', role: 'customer', isVerified: true });
  // Walk-in user — đại diện cho khách vãng lai mua tại quầy không có tài khoản
  await User.create({ name: 'Khách Vãng Lai', email: 'walkin@popcorn.local', password: 'walkin_not_login', role: 'customer', isVerified: false, isActive: true });
  ...
  const theater = await Theater.create({ name: 'Popcorn Cinema - Quận 1', address: '123 Nguyễn Huệ, Quận 1', city: 'Hồ Chí Minh', phone: '028-1234-5678' });
  const theater2 = await Theater.create({ name: 'Popcorn Cinema - Gò Vấp', address: '456 Quang Trung, Gò Vấp', city: 'Hồ Chí Minh', phone: '028-2345-6789' });
  const theater3 = await Theater.create({ name: 'Popcorn Cinema - Hà Nội', address: '789 Hoàng Cầu, Đống Đa', city: 'Hà Nội', phone: '024-1234-5678' });
  const theater4 = await Theater.create({ name: 'Popcorn Cinema - Đà Nẵng', address: '321 Nguyễn Văn Linh, Thanh Khê', city: 'Đà Nẵng', phone: '0236-1234-5678' });

  // Rooms
  const room1 = await Room.create({ theater: theater._id, name: 'Phòng 01 - Standard', type: 'standard', rows: 8, cols: 10, totalSeats: 80 });
  const room2 = await Room.create({ theater: theater._id, name: 'Phòng 02 - VIP', type: 'vip', rows: 6, cols: 8, totalSeats: 48 });
  ...
  const inserted1 = await Seat.insertMany(seatDocs1);
  // Đồng bộ vào room.seats embedded để SeatGrid hoạt động
  await Room.findByIdAndUpdate(room1._id, {
    seats: inserted1.map((s: any) => ({ _id: s._id, row: s.row, number: s.col, type: s.type, isAisle: false })),
    rows: 8, cols: 10,
  });
  ...
  const inserted2 = await Seat.insertMany(seatDocs2 as any[]);
  await Room.findByIdAndUpdate(room2._id, {
    seats: inserted2.map((s: any) => ({ _id: s._id, row: s.row, number: s.col, type: s.type, isAisle: false })),
    rows: 6, cols: 8,
  });
